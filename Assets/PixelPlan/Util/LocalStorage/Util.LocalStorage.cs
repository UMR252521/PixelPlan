        ...
        private class LocalStorage {
            ...
            private static T ReadItem<T>(string localStorageKey, T defaultValue, bool isObject){
                ...
                if(Instance.IsFileExistInPersistentDataPath(jsonFilePath)){
                    if(isObject){
                        return Util.Instance.ReadFromPersistentData<T>( jsonFilePath );
                    }else{
                        var typeData = Util.Instance.ReadFromPersistentData<LocalStorageTypeData<T>>( jsonFilePath );
            ...
            private static void WriteItem<T>(string localStorageKey, T value, bool isObject){
                if(isObject){
                    Util.Instance.WriteToPersistentData( JsonFilePath(localStorageKey), value );
                }else{
                    var wrapValue = new LocalStorageTypeData<T>();
                    wrapValue.data = value;
                    Util.Instance.WriteToPersistentData( JsonFilePath(localStorageKey), wrapValue );
                }
            }
        }
